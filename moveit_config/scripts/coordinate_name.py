...
class ObjectDetector:
    def __init__(self):
        ...
        self.target_name = rospy.get_param('~target_name', 'box_red')  # Default: box_red
        if not self.target_name:
            rospy.logwarn("Target name not provided, using default value 'box_red'")
            self.target_name = 'box_red'
        rospy.set_param('target_name', self.target_name)
    ...
    def image_callback(self, msg):
        ...
        # Detect contours of the target object based on its name
        target_name = "box_red"
        target_contour = None        
        # Convert BGR to HSV
        ...
        for contour in contours:
            # Assuming the target object is the largest contour with a specific name
            if cv2.contourArea(contour) > 1000 and rospy.get_param('target_name') == "box_red":
