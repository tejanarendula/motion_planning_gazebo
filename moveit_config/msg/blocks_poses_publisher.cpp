...
#include <moveit_config/blocks_poses.h>
...
int main(int argc, char** argv) {
    ...
    ros::Publisher poses_publisher = nh.advertise<moveit_config::blocks_poses>("blocks_poses", 1);
    moveit_config::blocks_poses current_poses_msg;

    //print out the current blocks poses
    ...
    while (ros::ok()) {
        if (g_poses_updated) {
            ...
            // Print the poses
            for (int i = 0; i < local_quantity; ++i) {
                 ROS_INFO_STREAM("Pose " << i << ": x=" << g_x[i] << ", y=" << g_y[i] << ", z=" << g_z[i]);
            }
        }
        ros::spinOnce();
    }
